...
struct NVSHMEMDataHandle {
  ...
  KOKKOS_INLINE_FUNCTION
  NVSHMEMDataElement<T> operator() (const int& pe, const iType& i) const {
...
struct NVSHMEMDataHandle<int> {
  ...
  template<typename iType>
  KOKKOS_INLINE_FUNCTION
  NVSHMEMDataElement<int> operator() (const int& pe, const iType& i) const {
...
struct ViewDataHandle<Traits,typename std::enable_if<std::is_same<typename Traits::specialize,NVSHMEMSpaceSpecializeTag>::value>::type> {
...
struct ViewTraits< void, NVSHMEMSpace , Prop ... >
{
  ...
  typedef typename NVSHMEMSpace::execution_space                   execution_space ;
  typedef typename NVSHMEMSpace::memory_space                      memory_space ;
  typedef typename Kokkos::Impl::HostMirror< NVSHMEMSpace >::Space HostMirrorSpace ;
...
template< class Traits >
class ViewMapping< Traits , NVSHMEMSpaceSpecializeTag >
{
private:
